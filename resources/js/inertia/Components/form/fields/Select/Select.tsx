...
function Select<TData>({
    ...
    return (
        <div className="relative w-full select-none" ref={fullContainer}>
            <label
                className={`block ${styles?.labelClasses ?? "font-medium text-gray-900 text-sm select-text"}`}
            >
                {label ?? ""}
                <input
                    ref={inputRef}
            ...
            <div
                onClick={() => handleOpen()}
                className={`flex justify-between cursor-pointer ${styles?.selectClasses ?? "border-gray-300 p-2 border rounded-lg w-full text-gray-700 sm:text-sm"}`}
            >
                    ...
                    {selected.length > 0 ? (
                        <div className="flex flex-wrap items-center gap-1">
                            {selected.map((option, index) => (
                                ...
                                >
                                    <span
                                        className={`${styles?.selectedItemsBadgeClasses ?? "bg-gray-500 hover:bg-red-400 p-0.5 rounded-sm text-white "} cursor-pointer`}
                                        onClick={(e) =>
                ...
                <div
                    className={
                        isOpen
                            ? `absolute left-0 z-50 ${styles?.dropDownItemsContainer ?? " px-3 pb-3 rounded-lg border border-gray-200 shadow-2xl bg-white w-full"}`
                            : "hidden"
                    }
                    style={{
                        top: `${
                ...
                >
                    <div className={`sticky top-0 bg-inherit`}>
                        <input
                            className={`${styles?.searchInputClasses ?? "focus:border-primary p-1 my-2 focus:outline-primary rounded-md w-full"}`}
                    ...
                    {data.map((item, index) => {
                        if (!search) {
                            return (
                                <div
                                    key={index}
                                    className={`${include(getOption(item), selected) ? `${styles?.selectedDropDownItemClasses ?? "bg-primary border-primary"}` : ""} ${styles?.dropDownItemClasses ?? "cursor-pointer hover:border-primary hover:bg-primary my-1 p-2 rounded-md w-full text-black"}`}
